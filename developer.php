...
if(isset($_POST['submitDeveloper'])){
...
<html lang="en">
  <head>
    <title>CONTRIBUTORS AND DEVELOPERS || CSC FUNAAB</title>
  ...
  <body id="top">
    ...
    <div class="sel_page">
    <?php 
		if(isset($_GET['action']) && $_GET['action']=='editDeveloper'){
			$page = 'developer';
		...
		echo '<a href="developer.php?action=updateDeveloper&id='. $result['id'] .'"><h3>'.$result['header'].'</h3></a>';
		
		}
			 ?>
              <p class="form_notice"><i>Please fill to add new  header/content</i></p>
        <form action="developer.php" method="post" id="form">
            ...
            <tr><td width="60px" height="50px"><label>Header</label></td><td><input type="text" name="header" size="74px" id="header"></td></tr>
              ...
              <tr><td colspan="2"><textarea cols="65" rows="15" name="note"></textarea></td></tr>
              <tr><td></td><td><input type="submit" name="submitDeveloper" id="submit" value="Insert Contributors and developers"></td></tr>
            </table>
            </form>
		<?php	}else 
			if(isset($_GET['action']) && $_GET['action']=='updateDeveloper'){
	 $id = $_GET['id'];
	 ...
	 <form action="developer.php" method="post" id="form">
            ...
            <tr><td width="60px" height="50px"><label>Header</label></td><td><input type="text" name="header" size="74px" id="header" value="<?php echo $result['header'] ?>"><input type="hidden" name="id" value="<?php echo $id ?>"/></td></tr>
              <tr><td colspan="2"><label>Content</label></td></tr>
              <tr><td colspan="2"><textarea cols="65" rows="15" name="note"><?php echo $result['note'] ?></textarea></td></tr>
              <tr><td></td><td><input type="submit" name="updateDeveloper" id="submit" value="Update contributors and developers">
              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="explore.php?action=del&id=<?php echo $result['id']; ?>" id="submit">Delete</a></td></tr>
            </table>
            </form>
	<?php
    }else{
		...
		echo $result['note'];
         ...
         <?php if(isset($_SESSION['username'])){ ?>
        <div class="editSection"><a href="developer.php?action=editDeveloper">Click To Edit Contributors and developers</a></div>
        <?php } ?>
    </div><!--sel_page-->
        </div><!--section-->
        </div><!-- end main -->
